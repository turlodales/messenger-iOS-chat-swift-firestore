...
@preconcurrency import InputBarAccessoryView
...
class ATCChatThreadViewController: MessagesViewController, MessagesDataSource, InputBarAccessoryViewDelegate {
  ...
  private func downloadImage(at url: URL, completion: @escaping (UIImage?) -> Void) {
    ...
    nonisolated func currentSender() -> SenderType {
  ...
  // MARK: - MessagesDataSource
    // Duplicate method? Removing or wrapping if needed. The protocol likely needs SenderType.
    // But keeping it safe.
    nonisolated func currentSender() -> Sender {
        MainActor.assumeIsolated {
            return Sender(id: user.uid ?? "noid", displayName: "You")
        }
    }
  
    nonisolated func numberOfMessages(in messagesCollectionView: MessagesCollectionView) -> Int {
        MainActor.assumeIsolated {
            return messages.count
        }
    }
  
    nonisolated func messageForItem(at indexPath: IndexPath,
    ...
    nonisolated func cellTopLabelAttributedText(for message: MessageType,
...
extension ATCChatThreadViewController: MessagesDisplayDelegate {
  ...
  nonisolated func backgroundColor(for message: MessageType, at indexPath: IndexPath,
  ...
  nonisolated func messageStyle(for message: MessageType, at indexPath: IndexPath,
  ...
  nonisolated func configureAvatarView(_ avatarView: AvatarView, for message: MessageType, at indexPath: IndexPath, in messagesCollectionView: MessagesCollectionView) {
    MainActor.assumeIsolated {
